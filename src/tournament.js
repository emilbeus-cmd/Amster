...
function thirdPlaceTieKey(row) {
  return `${row.points}|${row.goalDifference}|${row.goalsFor}`
}

function buildThirdPlaceRanking(tables) {
  const rows = groupIds
    .map((group) => ({
      group,
      seed: `${group}3`,
    ...
    .sort((a, b) => compareThirdPlaceRows(a.stats, b.stats) || groupIds.indexOf(a.group) - groupIds.indexOf(b.group))

  const tieGroups = new Map()
  rows.forEach((entry) => {
    const key = thirdPlaceTieKey(entry.stats)
    if (!tieGroups.has(key)) tieGroups.set(key, [])
    tieGroups.get(key).push(entry)
  })

  return rows.map((entry, index) => {
    const tiedEntries = tieGroups.get(thirdPlaceTieKey(entry.stats)) ?? []
    const tiedPositions = tiedEntries.map((candidate) => rows.indexOf(candidate) + 1)
    const isQualified = index < 8
    const unresolvedTie = tiedEntries.length > 1
    const affectsQualification = unresolvedTie && tiedPositions.some((position) => position <= 8) && tiedPositions.some((position) => position > 8)
    const affectsSlot = unresolvedTie && isQualified && tiedEntries.filter((candidate) => rows.indexOf(candidate) < 8).length > 1

    return {
      ...entry,
      rank: index + 1,
      qualified: isQualified,
      tiedGroups: tiedEntries.map((candidate) => candidate.group),
      unresolvedTie,
      affectsQualification,
      affectsSlot,
      note: unresolvedTie
        ? `Likt med gruppe ${tiedEntries.map((candidate) => candidate.group).join(', ')} på poeng, målforskjell og scorede mål. Fair play/loddtrekning er ikke hensyntatt.`
        : '',
    }
  })
}
...
function buildQualifiers(tables) {
  ...
  const thirdPlaced = buildThirdPlaceRanking(tables)
...
function assignThirdPlaceSlots(rankedThirds, slotDefinitions) {
  const slotsByCandidateCount = [...slotDefinitions].sort((a, b) => {
    const candidatesA = rankedThirds.filter((qualifier) => a.allowedGroups.includes(qualifier.group)).length
    const candidatesB = rankedThirds.filter((qualifier) => b.allowedGroups.includes(qualifier.group)).length
  ...
  function backtrack(slotIndex) {
    if (slotIndex >= slotsByCandidateCount.length) return true

    const slot = slotsByCandidateCount[slotIndex]
    const candidates = rankedThirds.filter((qualifier) => {
      const group = qualifier.group ?? qualifier.team?.group ?? qualifier.stats?.team?.group
      return group && slot.allowedGroups.includes(group) && !usedTeamIds.has(qualifier.team?.id)
    })

    for (const candidate of candidates) {
      assignments.set(slot.source, candidate)
      usedTeamIds.add(candidate.team.id)
      if (backtrack(slotIndex + 1)) return true
      usedTeamIds.delete(candidate.team.id)
      assignments.delete(slot.source)
    }
...
function buildRoundOf32(qualifiers) {
  ...
  const rankedThirds = Array.from({ length: 8 }, (_, index) => bySeed.get(`B3-${index + 1}`)).filter(Boolean)
  const thirdSlotDefinitions = [
    { source: '3ABCDF', allowedGroups: 'ABCDF' },
    { source: '3CDFGH', allowedGroups: 'CDFGH' },
    { source: '3CEFHI', allowedGroups: 'CEFHI' },
    { source: '3EHIJK', allowedGroups: 'EHIJK' },
    { source: '3BEFIJ', allowedGroups: 'BEFIJ' },
    { source: '3AEHIJ', allowedGroups: 'AEHIJ' },
    { source: '3EFGIJ', allowedGroups: 'EFGIJ' },
    { source: '3DEIJL', allowedGroups: 'DEIJL' },
  ]
  const thirdAssignments = assignThirdPlaceSlots(rankedThirds, thirdSlotDefinitions)

  const getSeed = (seed, source) => bySeed.get(seed) ?? placeholder(source)
  const getThird = (source) => {
    const third = thirdAssignments.get(source)

    if (!third?.team) return placeholder(source)
    return {
      ...third,
      source: `${third.source} · slot ${source}`,
    }
  }

  const roundOf32Slots = [
    ['M73', getSeed('A2', 'Toer gruppe A'), getSeed('B2', 'Toer gruppe B')],
    ['M74', getSeed('E1', 'Vinner gruppe E'), getThird('3ABCDF')],
    ['M75', getSeed('F1', 'Vinner gruppe F'), getSeed('C2', 'Toer gruppe C')],
    ['M76', getSeed('C1', 'Vinner gruppe C'), getSeed('F2', 'Toer gruppe F')],
    ['M77', getSeed('I1', 'Vinner gruppe I'), getThird('3CDFGH')],
    ['M78', getSeed('E2', 'Toer gruppe E'), getSeed('I2', 'Toer gruppe I')],
    ['M79', getSeed('A1', 'Vinner gruppe A'), getThird('3CEFHI')],
    ['M80', getSeed('L1', 'Vinner gruppe L'), getThird('3EHIJK')],
    ['M81', getSeed('D1', 'Vinner gruppe D'), getThird('3BEFIJ')],
    ['M82', getSeed('G1', 'Vinner gruppe G'), getThird('3AEHIJ')],
    ['M83', getSeed('K2', 'Toer gruppe K'), getSeed('L2', 'Toer gruppe L')],
    ['M84', getSeed('H1', 'Vinner gruppe H'), getSeed('J2', 'Toer gruppe J')],
    ['M85', getSeed('B1', 'Vinner gruppe B'), getThird('3EFGIJ')],
    ['M86', getSeed('J1', 'Vinner gruppe J'), getSeed('H2', 'Toer gruppe H')],
    ['M87', getSeed('K1', 'Vinner gruppe K'), getThird('3DEIJL')],
    ['M88', getSeed('D2', 'Toer gruppe D'), getSeed('G2', 'Toer gruppe G')],
  ]

  return roundOf32Slots.map(([id, home, away]) => ({
    id,
    round: 'round32',
    home,
    away,
...
function buildKnockoutBracket(roundOf32, winners) {
  ...
  const addMatch = (id, round, first, second) => {
    matches.push({
      id,
      round,
  ...
  round16Slots.forEach(([id, first, second]) => addMatch(id, 'round16', first, second))
  quarterSlots.forEach(([id, first, second]) => addMatch(id, 'quarter', first, second))
  semiSlots.forEach(([id, first, second]) => addMatch(id, 'semi', first, second))
  addMatch('M104', 'final', 'M101', 'M102')
